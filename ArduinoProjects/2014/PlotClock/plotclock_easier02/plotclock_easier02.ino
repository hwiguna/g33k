...
#define LIFT0 1250 // Hari:1220. Orig:1080 on drawing surface -- Larger# = closer to surface
#define LIFT1 1180  // Hari:1055. Orig:925  between numbers (park pen inside sweeper)
...
String Mode = "."; // .)stop D)raw, L)ift, R)ead, W)rite, G)o

double parkX = 73; //74.2; 
double parkY = 43; //47.5; Smaller# = closer to servo
...
void DisplayConfig() 
{
  ...
  Serial.println("r200 sets right0 to 200");
  Serial.println("r+10 increases right0 by 10");
  Serial.println("MD sets mode to Draw calibration");
  Serial.println(".)stop D)raw, V)ert, R)ead, W)rite, G)o");  
}
...
void HandleSerialCommands()
{
  if (stringComplete) {
    ...
    if (inputString.length() >= 1) {
      ...
      String val = inputString.substring(1, inputString.length()-1);
...
int NewValue(int oldValue, String val)
{
  int returnValue = oldValue;
  
  if (val[0]=='-')
    returnValue += val.toInt();
  else if (val[0]=='+') 
    returnValue += val.toInt();
  else
    returnValue = val.toInt();
...
void ProcessCommand(String cmd, String val) {
  ...
  //int value = val.toInt();
  switch (cmd[0]) {
    case 'l': Serial.println("left 0");   Left0   = NewValue(Left0, val); break;
    case 'L': Serial.println("LEFT 90");  Left90  = NewValue(Left90, val); break;
    case 'r': Serial.println("right 0");  Right0  = NewValue(Right0, val); break;
    case 'R': Serial.println("RIGHT 90"); Right90 = NewValue(Right90, val); break;

    case 'x': Serial.println("ParkX"); parkX = NewValue(parkX, val); break;
    case 'y': Serial.println("ParkY"); parkY = NewValue(parkY, val); break;

    case '0': Serial.println("Lift 0"); lift0 = NewValue(lift0, val); break;
    case '1': Serial.println("Lift 1"); lift1 = NewValue(lift1, val); break;
    case '2': Serial.println("Lift 2"); lift2 = NewValue(lift2, val); break;

    case 'm': 
    case 'M': Serial.println("Mode");     Mode = val; break;
    default:
      isBadCmd = true;
      break;
  }
...
{ 
  ...
  // .)stop D)raw, V)ert, R)ead, W)rite, G)o
  switch (Mode[0]) {
    case 'g': case 'G': ClockMode(); break;
    case 'd': case 'D': CalibrateDraw(); break;
    case 'v': case 'V': CalibrateLift(); break;
    case 'r': case 'R': break;
    case 'w': case 'W': break;
  }
}
