...
const PROGMEM uint8_t font[96][6] = {
...
void test()
{
  ...
  {
    uint8_t vertByte = pgm_read_byte_near(font + ch*8 + x);
    for (uint8_t y = 0; y <= 7; y++)
    {
      Serial.print(bitRead(vertByte,y));
      
      //if (bitRead(vertByte,y)) PORTD = bitAnalogValue; else PORTD = 0;
    }
    Serial.println();
  }
    Serial.println();
