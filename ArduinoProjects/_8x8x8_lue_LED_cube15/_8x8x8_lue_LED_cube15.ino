...
void loop(void) {
  ...
  //while (true) AnimatedDot();
    //while (true) Tornado();
//    FleaJump(1,1, 6,1, 2); // Front
//    FleaJump(6,1, 6,7, 3); // Right
...
//      if (receivedCount==128) SetCubeByString(1, buffer);
//
//      receivedCount = Serial.readBytesUntil('\n', buffer, 128);
//      Serial.print(receivedCount);  Serial.print(", "); 
//      if (receivedCount==128) SetCubeByString(2, buffer);
//
//      receivedCount = Serial.readBytesUntil('\n', buffer, 128);
//      Serial.println(receivedCount); 
