...
//v11 - Added Clear(), and Print() to display non-scrolling text. Also added numbers and some symbols.
  ...
  for (int charIndex=0; charIndex < msg.length(); charIndex++)
  {
    ...
    for (int col = 0; col < 7; col++)
    {
      for (int row = 0; row < 8; row++)
      {
        ...
        bool isOn = 0; 
        if (col<5) isOn = bitRead( alphabets[alphabetIndex][col], 7-row ) == 1;
        Plot( (charOffset+charIndex)*7 + col, row, isOn); // We ALWAYS draw on the rightmost column, the shift loop below will scroll it leftward.
      }
    }
  }
  ...
  if (drag-- <= 0) {
    ...
    switch (counter--) {
      ...
      case -1:
        counter = 10;
        Clear();
        Print(0, " DOOM:" + String(counter--) );
        break;
      default:
        Print(6, String(counter) + " ");
    }
  }  
}
...
void loop() {
  //-- Scrolling display example --
  //AlphabetSoup();
  
  //-- NON-scrolling display example --
  Countdown(); // Uses Clear() and Print() to display non-scrolling text
