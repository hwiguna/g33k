...
void setup() {
  ...
  if (debug) Serial.println(F("Adafruit IO Example"));

  // Connect to WiFi access point.
  //Serial.println(); Serial.println();
  delay(10);
  if (debug) Serial.print(F("Connecting to "));
...
void checkEvents()
{
  ...
  while (subscription = mqtt.readSubscription(1000)) {
    ...
    if (debug) Serial.println(F("Incoming!"));
...
void publishPhotocell()
{
  ...
  if (abs(currentA - lastA) > 5)
  {
    ...
    if (debug) Serial.print(currentA);
    if (debug) Serial.print("... ");
...
void connect() {
  ...
  if (debug) Serial.print(F("Connecting to Adafruit IO... "));
...
if (debug) 
    ...
    if (debug) Serial.println(F("Retrying connection..."));
    delay(5000);

  }

  if (debug) Serial.println(F("Adafruit IO Connected!"));
