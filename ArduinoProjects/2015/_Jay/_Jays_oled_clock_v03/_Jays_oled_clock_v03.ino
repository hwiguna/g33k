/*
  ...
  v00 - Format the output of RTCZero into a nice padded string
  v01 - Put RTCZero and OLED code adapted to work on the Arduino M0 into one sketch
  v02 - Even larger fonts
  V03 - Run on a Nano with RTC instead of Zero and no rtc.
...
//  {
//    for (byte r = 0; r < 2; r++) // Draw each row twice (one below the other)
//    {
//      OLED_SetColumnAddress_128128RGB(x_pos, 0x7F);
//      OLED_SetRowAddress_128128RGB(y_pos + i * 2 + r, 0x7F);
//      OLED_WriteMemoryStart_128128RGB();
//      for (count = 0; count < 5; count++) // each character is 10 pixels wide
//      {
//        if ((Ascii_1[letter][count] & mask) == mask) {
//          OLED_Pixel_128128RGB(textColor);
//          OLED_Pixel_128128RGB(textColor);
...
void OLED_TextX_128128RGB(unsigned char x_pos, unsigned char y_pos, unsigned char letter, unsigned long textColor, unsigned long backgroundColor, byte mag)
{
  int i;
  int count;
  unsigned char mask = 0x80;
  ...
  for (i = 0; i < 8; i++) // each character is 8 pixels tall
  {
    for (byte r = 0; r < mag; r++) // Draw each row twice (one below the other)
    {
      OLED_SetColumnAddress_128128RGB(x_pos, 0x7F);
      OLED_SetRowAddress_128128RGB(y_pos + i * mag + r, 0x7F);
      OLED_WriteMemoryStart_128128RGB();
      for (count = 0; count < 5; count++) // each character is 10 pixels wide
      {
        if ((Ascii_1[letter][count] & mask) == mask) {
...
void setup_oled()                                 // for Arduino, runs first at power on
{
...
void PrintTiny(int x, int y, String str)
{
...
void PrintNormal(int x, int y, String str)
{
  for (int i = 0; i < str.length(); i++)
  {
    OLED_TextX_128128RGB(x + (i*7*2),y, str[i]-32, GREEN, BLACK, 2);
  }
}

void PrintBig(int x, int y, String str)
{
  for (int i = 0; i < str.length(); i++)
  {
    OLED_TextX_128128RGB(x + (i * 7 * 2), y, str[i] - 32, GREEN, BLACK, 5);
  }
}
...
void loop()
{
  ...
  if (newSeconds != prevSeconds)
  {
    ...
    String timeStr = Pad(now.hour()) + ":" + Pad(now.minute()) + ":" + Pad(now.second());
    //Serial.println(timeStr);

    //-- Print to OLED --
    PrintNormal(10, 64 - 20 - 20, dateStr);
    PrintBig(10, 64 - 4 - 16, timeStr);
  }
